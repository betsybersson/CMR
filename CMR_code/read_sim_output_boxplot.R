...
cov.method = "kron"  # "cor9"# "comSym3groups"  #cor9
plottitle = expression(Sigma[kron]) #expression("(a) " ~ Sigma[cor]) # expression("(a) " ~ Sigma[cor]) # expression("(b) " ~ Sigma[block]) # 
legendposit =  "bottom"  # "" #
plotidentifier = ""

identifier = "saveall_difTrueMatrix" #"saveall"
ps = c("9","16","50") 
if (cov.method == "kron" & identifier == "saveall"){
  ps = c("9","15","51")
} else if (cov.method == "kron" & identifier == "saveall_difTrueMatrix"){
  ps = c("8","15","50")
}
...
for ( p.ind in 1:length(ps)){
  ...
  file.name = paste0("output/SIM_",cov.method,"_p",ps[p.ind],"_",identifier,".Rdata")
  load(file.name)
  ...
  data_frame$Method = factor(data_frame$Method, 
                             levels = c("MR.C","MR.I","MLE","Kron","CUSP","SIS"))
  
  # pdf(filename = , family="Times",height=6,width=6)
  # ggplot(data_frame) +
  ...
  #   ylab("Log Loss") +
  #   scale_color_manual(values = c("MR.O" = "red", "MLE" = "blue",
  #                                 "CUSP" = "darkgreen", "SIS" = "purple",
  #                                 "MR.I" = "orange", "Kron" = "black"))
  # dev.off()
...
pdf(paste0("./plots/Loss_boxplots_",cov.method,plotidentifier,".pdf"), family="Times",
    height=8,width=8) # height=7.6 # for cor9
ggplot(grand.df) +
  geom_boxplot(aes(y=Loss,color=Method)) +
  ...
  ylab("Log Loss") + 
  ggtitle(plottitle) +
  scale_color_manual(values = c("MR.C" = "red", "MLE" = "blue",
                                "CUSP" = "darkgreen", "SIS" = "purple",
                                "MR.I" = "orange", "Kron" = "black"))
dev.off()
